...
public:
    ...
    bool isBinarySearchTree()const{
        return isBinarySearchTree(root_, nullptr, nullptr);
    }
...
private:
    ...
    bool isBinarySearchTree(const Node* node, const Node* minNode, const Node* maxNode)const{
        if(!node) return true;

        if((minNode && node->key_ <= minNode->key_) || (maxNode && node->key_ >= maxNode->key_)){
            return false;
        }
        return isBinarySearchTree(node->left_, minNode, node) 
        && isBinarySearchTree(node->right_, node, maxNode);
    }
};
